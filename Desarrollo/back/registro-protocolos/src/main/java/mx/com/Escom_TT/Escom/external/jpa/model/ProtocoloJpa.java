...
public class ProtocoloJpa {
    ...
    public static ProtocoloJpa fromEntity(Protocolo protocolo){
        return ProtocoloJpa.builder().nombre(protocolo.getNombre())
                .fechaEntrega(protocolo.getFechaEntrega())
                .idProtocolo(protocolo.getIdProtocolo()).idEstado(protocolo.getIdEstado())
                .primerDirector(protocolo.getPrimerDirector()).segundoDirector(protocolo.getSegundoDirector()).registro(protocolo.getRegistro()).build();
    }

    public Protocolo toEntity(){
        return Protocolo.builder().idProtocolo(this.idProtocolo)
                .nombre(this.nombre).fechaEntrega(this.fechaEntrega)
                .primerDirector(this.primerDirector).segundoDirector(this.segundoDirector).registro(this.registro).build();
    }
