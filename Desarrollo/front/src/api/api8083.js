...
const api8083 = axios.create({
  baseURL: "http://172.17.0.1:8083", // URL base del backend para el puerto 8082
  timeout: 10000, // Tiempo máximo para una solicitud (en milisegundos)
});

// Función para enviar multipart/form-data
api8083.sendFormData = async (url, formData) => {
  try {
    const response = await api8083.post(url, formData, {
      headers: {
...
api8083.getData = async (url, params = {}) => {
  try {
    const response = await api8083.get(url, { params });
    return response;
  } catch (error) {
